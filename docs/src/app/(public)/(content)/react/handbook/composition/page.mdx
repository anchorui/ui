...
<Subtitle>A guide to composing Anchor UI components with your own React components.</Subtitle>
<Meta
  name="description"
  content="A guide to composing Anchor UI components with your own React components."
/>

## Composing custom React components

Use the `render` prop to compose an Anchor UI part with your own React components.
...
In situations where you need to compose multiple Anchor UI components with custom React components, `render` props can be nested as deeply as necessary.
  ...
  <Menu.Root>
    ...
    <Menu.Portal>
      <Menu.Positioner>
        <Menu.Popup>
          {/* prettier-ignore */}
          <Menu.Item render={<a href="anchorui.com" />}>
            Add to Library
          </Menu.Item>
        </Menu.Popup>
      </Menu.Positioner>
...
Each Anchor UI component renders the most appropriate element by default, and in most cases, rendering a different element is recommended only on a case-by-case basis.
