# Direction Provider

<Subtitle>Enables RTL behavior for Anchor UI components.</Subtitle>
<Meta
  name="description"
  content="A direction provider component that enables RTL behavior for Anchor UI components."
/>

<Demo path="./demos/hero" />

## API reference

Import the component and wrap it around your app:

```jsx title="Anatomy"
import { DirectionProvider } from '@anchor-ui/react/direction-provider';

// prettier-ignore
<DirectionProvider>
  {/* Your app or a group of components */}
</DirectionProvider>
```

<Reference component="DirectionProvider" />
