...
function resolveDependencies(packageName: string): Record<string, string> {
  switch (packageName) {
    case '@anchor-ui/react': {
      if (COMMIT_REF === undefined || SOURCE_CODE_REPO !== 'https://github.com/anchorui/ui') {
        // #default-branch-switch
        return {
          '@anchor-ui/react': 'latest',
        };
      }
      const shortSha = COMMIT_REF.slice(0, 8);
      return {
        '@anchor-ui/react': `https://pkg.csb.dev/ipushpendra/anchor-ui/commit/${shortSha}/@anchor-ui/react`,
      };
    }
